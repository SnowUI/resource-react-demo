    ...
    <header className="sticky top-0 z-20 p-5 flex items-center justify-between py-3 border-b border-black-10 bg-background-1">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col">
          {/* 主标题：SnowUI */}
          <span className="text-base leading-6 text-foreground">SnowUI Design Resource</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className="flex items-center p-1 gap-1 px-1 rounded-xl text-foreground transition-colors hover:border-[var(--primary)]"
          aria-label={isLightMode ? t('theme.dark', language) : t('theme.light', language)}
        >
          {isLightMode ? <Sun size={16} /> : <Moon size={16} />}
          <span className="text-xs leading-4">{isLightMode ? t('theme.light', language) : t('theme.dark', language)}</span>
        </button>
        ...
        >
          ...
          <span className="text-xs leading-4">{isSnowUITheme ? t('theme.snowui', language) : t('theme.ios', language)}</span>
        </button>

        {/* ========== 语言选择器 ========== */}
        {/* 功能：切换网站语言 */}
        {/* 显示：地球图标 + 语言缩写下拉选择框 */}
        <div className="flex items-center p-1 gap-0 px-1 rounded-xl ]">
          <Globe size={16} className="text-foreground" />
          <select
            value={language}
            onChange={(event) => setLanguage(event.target.value as typeof language)}
            className="bg-transparent border-none outline-none text-xs leading-4 text-foreground"
          >
            {languages.map((lang) => (
