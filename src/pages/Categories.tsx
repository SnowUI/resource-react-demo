...
const Categories: React.FC = () => {
  ...
  const renderInfoText = () => {
    ...
    if (categoryKey === 'icons') {
      return (
        <p className="text-xs leading-4 text-black-40">
  ...
  const renderAsset = (item: AssetItem) => {
    switch (categoryKey) {
      ...
      case 'illustrations':
        return (
          <div className="w-full p-5 aspect-video rounded-lg overflow-hidden bg-background-2 flex items-center justify-center">
    ...
    <div className="flex-1 p-6 md:p-8">
      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-lg leading-7 font-semibold text-foreground mb-2">
        ...
        <div className={`grid ${gridClassByCategory[categoryKey]} gap-6`}>
          {paginatedItems.map((item) => (
            ...
            >
              ...
              {/* 素材名称文字 */}
              {/* 悬停时文字颜色变化 */}
              <span className="text-xs leading-4 text-black-40 text-center break-all group-hover:text-[var(--foreground)] transition-colors">
                {item.name}
              </span>
            </div>
          ))}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-3 mt-12 mb-6">
            ...
            <button
              ...
              disabled={page === 1}
              className="px-4 py-2.5 rounded-xl bg-background-2 border border-black-10 text-foreground disabled:opacity-50 disabled:cursor-not-allowed hover:border-[var(--primary)] transition-colors"
            >
              Prev
            </button>
            {/* 页码显示 */}
            <span className="text-sm leading-5 text-black-40">
              {page} / {totalPages}
            </span>
            {/* 下一页按钮 */}
            <button
